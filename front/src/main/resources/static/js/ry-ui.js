/**
 * 通用方法封装处理 Copyright (c) 2018 ruoyi
 */
(function($) {
	$.extend({
		// 表格封装处理
		table : {
			...
			init : function(options) {
				...
				$selecter = options.selecter == undefined ? '#bootstrap-table' : options.selecter;
				$($selecter).bootstrapTable({
			...
			search : function(form) {
				...
				params.queryParams = function(paramz) {
					var search = {};
					search.pageSize = paramz.limit;
					search.pageNum = (paramz.offset / paramz.limit) + 1;
					$.each($('#' + form).serializeArray(), function(i, field) {
						if(field.value != null && field.value != '' && field.name != 'startDate' && field.name != 'endDate'){
							search[field.name] = field.value;
						}else if(field.value != null && field.value != '' && (field.name == 'startDate' || field.name == 'endDate')){
							search[field.name] = new Date(field.value);
						}
					});
					search.searchValue = paramz.search;
					search.orderByColumn = paramz.sort;
					search.isAsc = paramz.order;
					return search;
				}
			...
			exportExcel : function() {
				layer.confirm("确认下载excel?", {
					btn: ['确认', '取消']
				}, function(index) {
					layer.close(index);
					//console.log('下载');
					//console.log($.table._option.exportUrl);
					...
					form.action = $.table._option.exportUrl;
		...
		operate : {
			...
			// 添加日常报销单 需补充
			addNormal : function(flag, planId, type, expensesType) {
				//console.log(flag);
				//console.log($.table._option.addExpensesUrl);
				//console.log($("#expensesType").val());
				if(flag === 1){
					$.modal.openFull($.table._option.modalName, $.table._option.addExpensesUrl + '?expensesType='+$('#expensesType').val());
				} else if(flag === 2) {
					$.modal.openFull("日常费用查看", $.table._option.printExpensesUrl+'?planId='+planId+'&type='+type+'&expensesType='+expensesType);
				} else if(flag === 3) {
					$.modal.openFull("日常费用打印", $.table._option.printExpensesUrl+'?planId='+planId+'&type='+type+'&expensesType='+expensesType);
				} else if(flag === 4) {
					$.modal.openFull("会议费用查看", $.table._option.printExpensesForMeetingUrl+'?planId='+planId+'&type='+type+'&expensesType='+expensesType);
				} else if(flag === 5) {
					$.modal.openFull("会议费用打印", $.table._option.printExpensesForMeetingUrl+'?planId='+planId+'&type='+type+'&expensesType='+expensesType);
				} else if(flag === 6) {
					$.modal.openFull("会议费用创建", $.table._option.addExpensesForMeetingUrl+'?expensesType='+$('#expensesType').val());
				} else if(flag === 7) {
					var _url;
					if($("#expensesType").val() == 1 || $("#expensesType").val() == 2 || $("#expensesType").val() == 3){
						_url = '../org/orgTreePage.do?orgIdIn=' + $("#orgIdIn").val() + '&DQ=Y';
								
					}else {
						_url = '../org/orgTreePage.do?orgIdIn=' + $("#orgIdIn").val();
					}
					$.modal.openFull("选择组织", _url);
				}
